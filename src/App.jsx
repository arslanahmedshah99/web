...
const AuthPage = ({ onLogin }) => {
  ...
  const getUsers = () => {
    const saved = window.localStorage.getItem("fixMyCampusUsers");
  ...
  const handleRegister = () => {
    ...
    setTimeout(() => {
      const users = getUsers();
      if (users.some(u => u.email === email)) { setError("Email already registered."); setLoading(false); return; }
      const newUser = { email, name, role, password };
      window.localStorage.setItem("fixMyCampusUsers", JSON.stringify([...users, newUser]));
      onLogin(newUser);
      setLoading(false);
    }, 700);
  };
    ...
    <div style={{
      ...
      <div style={{ width: "100%", maxWidth: 420 }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          ...
          <div style={{ fontSize: 22, fontWeight: 700, color: "#0C447C" }}>fixcampus</div>
        </div>
          ...
          {tab === 'login' ? (
            <div>
              ...
              <div style={{ marginTop: 18, background: "#F1EFE8", borderRadius: 9, padding: "12px 14px" }}>
                <div style={{ fontSize: 11, fontWeight: 600, color: "#444441", marginBottom: 7 }}>Available accounts</div>
                {getUsers().map(u => (
...
const IssuesList = ({ issues, onStatusChange, userRole }) => {
            ...
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 8, flexShrink: 0 }}>
                ...
                {isAdmin ? (
                  ...
                  <div style={{ fontSize: 11, color: "#B4B2A9" }}>Admin only</div>
                )}
              </div>
            </div>
          </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, maxWidth: 680 }}>
        ...
        <Field label="Room / Specific location">
          <input
            ...
            autoComplete="off"
            placeholder="e.g. Room 204, near main entrance"
      ...
      <div style={{ maxWidth: 680 }}>
        <Field label="Description *" error={errors.desc}>
          <textarea
            ...
            onFocus={() => clearError("desc")}
            spellCheck="false"
            rows={4}
            placeholder="Describe the issue clearly. What is broken? When did it start? Is it a safety risk?"
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
        {tips.map(t => (
          <div key={t.title} style={{ background: "#fff", border: "0.5px solid #e5e3db", borderRadius: 12, padding: "16px 18px" }}>
            ...
            <div style={{ fontSize: 13, fontWeight: 500, color: "#2C447C", marginBottom: 5 }}>{t.title}</div>
...
const AdminPanel = () => {
  ...
  const saveUsers = (updatedUsers) => {
    ...
    window.localStorage.setItem("fixMyCampusUsers", JSON.stringify(updatedUsers));
  };
  ...
  const handleRemoveUser = (email) => {
    if (email === "admin@campus.ac.uk") {
      setError("Cannot remove the default admin user.");
      return;
    }
  ...
  return (
    <div style={{ padding: "24px 28px" }}>
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ fontSize: 22, fontWeight: 600, color: "#1a1a18" }}>Admin Panel</h1>
        <p style={{ fontSize: 13, color: "#888780", marginTop: 4 }}>Manage users and system settings.</p>
      </div>

      {error && (
        <div style={{ background: "#FCEBEB", border: "0.5px solid #F7C1C1", borderRadius: 8, padding: "10px 14px", marginBottom: 16, fontSize: 12, color: "#791F1F" }}>
          ⚠️ {error}
        </div>
      )}

      <div style={{ background: "#fff", border: "0.5px solid #e5e3db", borderRadius: 12, padding: "20px", marginBottom: 24 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          <h2 style={{ fontSize: 16, fontWeight: 600, color: "#1a1a18", margin: 0 }}>Users</h2>
          <button onClick={() => { setShowForm(!showForm); setError(""); }} style={{ padding: "8px 16px", borderRadius: 8, background: "#185FA5", color: "#fff", border: "none", cursor: "pointer", fontSize: 12, fontWeight: 600 }}>
            {showForm ? "Cancel" : "+ Add User"}
          </button>
        </div>

        {showForm && (
          <div style={{ background: "#F1EFE8", borderRadius: 10, padding: "16px", marginBottom: 16 }}>
            <div style={{ marginBottom: 12 }}>
              <label style={{ display: "block", fontSize: 12, fontWeight: 500, color: "#444441", marginBottom: 4 }}>Name</label>
              <input type="text" value={newUser.name} onChange={e => setNewUser(u => ({ ...u, name: e.target.value }))} placeholder="Full name" style={{ width: "100%", padding: "8px 12px", borderRadius: 6, border: "0.5px solid #D3D1C7", fontSize: 12, outline: "none" }} />
            </div>
            <div style={{ marginBottom: 12 }}>
              <label style={{ display: "block", fontSize: 12, fontWeight: 500, color: "#444441", marginBottom: 4 }}>Email</label>
              <input type="email" value={newUser.email} onChange={e => setNewUser(u => ({ ...u, email: e.target.value }))} placeholder="user@campus.ac.uk" style={{ width: "100%", padding: "8px 12px", borderRadius: 6, border: "0.5px solid #D3D1C7", fontSize: 12, outline: "none" }} />
            </div>
            <div style={{ marginBottom: 12 }}>
              <label style={{ display: "block", fontSize: 12, fontWeight: 500, color: "#444441", marginBottom: 4 }}>Password</label>\n              <input type="password" value={newUser.password} onChange={e => setNewUser(u => ({ ...u, password: e.target.value }))} placeholder="••••••••" style={{ width: "100%", padding: "8px 12px", borderRadius: 6, border: "0.5px solid #D3D1C7", fontSize: 12, outline: "none" }} />
            </div>
            <div style={{ marginBottom: 16 }}>
              <label style={{ display: "block", fontSize: 12, fontWeight: 500, color: "#444441", marginBottom: 4 }}>Role</label>
              <select value={newUser.role} onChange={e => setNewUser(u => ({ ...u, role: e.target.value }))} style={{ width: "100%", padding: "8px 12px", borderRadius: 6, border: "0.5px solid #D3D1C7", fontSize: 12, outline: "none" }}>
                <option>Student</option>\n                <option>Staff</option>\n                <option>Admin</option>\n              </select>
            </div>
            <button onClick={handleAddUser} style={{ width: "100%", padding: "10px", borderRadius: 8, background: "#185FA5", color: "#fff", border: "none", cursor: "pointer", fontSize: 12, fontWeight: 600 }}>Add User</button>
          </div>
        )}

        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {users.map(u => (
            <div key={u.email} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px", background: "#F7F6F2", borderRadius: 8, border: "0.5px solid #e5e3db" }}>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 13, fontWeight: 500, color: "#2C2C2A" }}>{u.name}</div>
                <div style={{ fontSize: 11, color: "#888780", marginTop: 2 }}>{u.email} • {u.role}</div>
              </div>
              <button
                onClick={() => handleRemoveUser(u.email)}
                disabled={u.email === "admin@campus.ac.uk"}
                style={{
                  padding: "6px 12px", borderRadius: 6, border: "1px solid #F09595", background: u.email === "admin@campus.ac.uk" ? "#f5f5f5" : "#fff",
                  color: u.email === "admin@campus.ac.uk" ? "#ccc" : "#791F1F", cursor: u.email === "admin@campus.ac.uk" ? "not-allowed" : "pointer",
...
export default function App() {
  const [user, setUser] = useState(() => {
    const saved = window.localStorage.getItem("fixMyCampusUser");
    return saved ? JSON.parse(saved) : null;
  });
  const [page, setPage] = useState("dashboard");
  const [issues, setIssues] = useState(() => {
    const saved = window.localStorage.getItem("fixMyCampusIssues");
    return saved ? JSON.parse(saved) : ISSUES_DATA;
  });

  useEffect(() => {
    if (user) {
      window.localStorage.setItem("fixMyCampusUser", JSON.stringify(user));
    } else {
      window.localStorage.removeItem("fixMyCampusUser");
    }
  }, [user]);

  useEffect(() => {
    window.localStorage.setItem("fixMyCampusIssues", JSON.stringify(issues));
  }, [issues]);

  const handleLogin = (userData) => {
    const token = `demo-jwt-${Math.random().toString(36).slice(2)}-${Date.now()}`;
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", fontFamily: "'DM Sans', 'Segoe UI', sans-serif", background: "#F7F6F2" }}>
      <div style={{ width: 210, background: "#fff", borderRight: "0.5px solid #e5e3db", display: "flex", flexDirection: "column", flexShrink: 0 }}>
        <div style={{ padding: "18px 16px 14px", borderBottom: "0.5px solid #e5e3db" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <div>
              <div style={{ fontSize: 13, fontWeight: 600, color: "#1a1a18" }}>fixcampus</div>
            </div>
          </div>
        </div>
        ...
        <div style={{ padding: "12px 16px", borderTop: "0.5px solid #e5e3db", display: "flex", flexDirection: "column", gap: 10 }}>
          <div style={{ fontSize: 11, color: "#B4B2A9" }}>Logged in as {user.name || user.email}</div>
          <button onClick={handleLogout} style={{ width: "100%", padding: "9px 12px", borderRadius: 8, border: "1px solid #D3D1C7", background: "#fff", color: "#5F5E5A", cursor: "pointer", fontSize: 12, fontWeight: 600 }}>
            Logout
          </button>
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflowY: "auto" }}>
        ...
        {page === "issues" && <IssuesList issues={issues} onStatusChange={handleStatusChange} userRole={user.role} />}
